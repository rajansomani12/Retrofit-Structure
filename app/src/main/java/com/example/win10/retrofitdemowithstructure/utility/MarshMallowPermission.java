...
/**
 * Created by hitendra on 11/4/18.
 */
...
public class MarshMallowPermission {
    ...
    public static final int INTERNET_ACCESS_REQUEST_CODE = 5;
    Activity activity;
    Context mContext;
    ...
    public void requestforInterNet() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(activity, Manifest.permission.INTERNET)) {
            Toast.makeText(mContext.getApplicationContext(), "Internet is Needed. Please allow in App Settings for additional functionality.", Toast.LENGTH_LONG).show();
        } else {
            ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.INTERNET}, INTERNET_ACCESS_REQUEST_CODE);
        }
    }
}